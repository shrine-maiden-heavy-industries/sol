...
# Adapted from: https://codeberg.org/Squishy/squishy/src/branch/main/squishy/core/pcapng/__init__.py
...
# NOTE(aki): We can't use `build_stream` in the `write_*` because construct wants to seek around,
...
class PcapngStream:
	...
	def emit_interface(self, type: LinkType, name: str, *, snap_len: int = 0, options: Iterable = ()) -> PcapngInterface:
		'''
		...
		write_idb(self._data, type, snap_len, name, options = options)
		self._data.flush()
		self._last_interface += 1
		return PcapngInterface(_id = self._last_interface, _name = name, _type = type, _data = self._data)
