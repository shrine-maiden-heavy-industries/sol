...
Hey! Thanks for your interest in contributing to SOL!

A few important things to note up front:

* Contributions to the SOL source code are licensed under the [BSD-3-Clause], the full text of which can be found in the [`LICENSE`] file in the root of the [git repository].
* Contributions to the SOL documentation are licensed under the [CC-BY-SA 4.0], the full text of which can be found in the [`LICENSE.docs`] file in the root of the [git repository].
* Contributions to the SOL hardware are licensed under the [CERN-OHL-P v2], the full text of which can be found in the [`LICENSE.hardware`] file in the root of the [git repository].
* When contributing to SOL and interacting with people in/around the project be sure to keep in mind the [Code Of Conduct].
...
The goal for SOL is to be able to be as friendly and powerful as possible, allowing for anyone of any skill level to get into hardware design, as such it is extremely important that we get feedback from users so we know what snags, sharp corners, unclear information, and unexpected behavior they encounter.

It is asked that you please report any and all problems you encounter when using SOL, this can be from small things such as typos or weird wording, and unclear/confusing messages and diagnostics, to larger things such as general useability and tool integration.

Reporting an issue with SOL is done through the [issue tracker], the issue templates should guide you through most of the needed information. A quick summary of what would be needed is as follows:

* The version of SOL being used, this can be done by running `python -c "import sol_usb;print(sol_usb.__version__)"`
...
We are trying to make SOL as widely useable and capable as possible, as such we love hearing about things that would help us achieve that goal.
...
The guidelines and instructions below are a best-effort attempt at describing the least-friction way of getting started with doing development and testing for SOL. If you notice any problems then please do [report an issue], and if you need any help then feel free to use the [discussion forum] and ask there.

### Required Tools

When working with the SOL code base, we require the following tools:
...
Other than that, your code editor and everything else is up to personal taste, so go ham. If you are having a hard time finding things like [Yosys], [sby], [Yices2], and [Bitwuzla], those, along with a whole lot of other tools are in the [OSS CAD Suite] builds. Instructions in that repository should help you get those set up so you can use them with SOL.

### Setting Up

1. [Fork] the repository to your GitHub account and clone it locally if you've not done so already previously.
1. Within the local repository, add this repo as a remote: `git remote add upstream https://github.com/shrine-maiden-heavy-industries/sol`
...
The overall style guidelines for SOL are not super fixed or well defined at the moment, but the following things are required:
...
To run the SOL test suite, you can simply invoke `nox -s test`, this will do everything needed to set up SOL and then invoke it's test suite on the current state of the code base as it is locally.
...
* If the commit touches part of the core SOL package, try to use the full dotted path to the module. (e.g. `sol_usb.usb`, `sol_usb.interface.pipe`, etc. )
...
```text
docs: Fixed a pile of typos in applet guide
```

```text
sol_usb.gateware.applets: Added USB PD Debugging Applet

Added an applet to allow for sniffing USB PD traffic.
```
...
Next, the `$function` section can be thought of something like a filesystem path segment that describes where this branch is focused, for instance, `gateware/platform` if it's related to hardware platforms. If the changes are more wide-reaching than a specific sub-section, then this part can be omitted.

Finally, the `$name` section should be a short, but descriptive name for what this branch is doing, it doesn't need to be super detailed, that's what commit messages are for, but something that gives a general idea, like `ulpi-phy-registers`.

So, to put it all together, the following are good examples of how your branch should be named:

* `fix/gateware/platform/ulpi-phy-registers`
* `docs/grammar-fixes`

### Submitting a Pull Request

> [!IMPORTANT]
> SOL uses a [git rebase] based workflow to maintain a linear history and avoid merge commits.

Once your changes are done, tested, and you're happy with them, then now to open a pull request against the SOL repository.
...
Once your changes are all up to date, make sure that all the tests pass (see the [Testing] section for more info), and then you can then open a [new pull request] against the SOL repository.
...
And with that, you're done! You've contributed to SOL!

## Documentation

The documentation for SOL uses [sphinx] with a few plugins, most notably [MyST], which allows us to use Markdown with some extensions to do the documentation rather than reStructuredText.

When contributing to the SOL documentation there are 2 key places it can be found, the "free text" documentation is located in the [`docs`] directory in the root of the repository and contains all the main markup for the overall layout and contents of the documentation site, where as the API and most library documentation is done via Python doc comments in the relevant source code itself.
...
This policy covers ***ALL*** parts of this project, including but not limited to code, tests, documentation, metadata, artworks, issues, comments, suggestions, feature requests, discussions, pull requests, and any other possible contributions that can be made to SOL.

If you are found to be using such tools when contributing to SOL, all of you contributions will be removed if viable, outstanding contributions closed without recourse or review, all future collaboration efforts denied, and you will be banned from contributing to SOL and related projects.
...
[`LICENSE`]: https://github.com/shrine-maiden-heavy-industries/sol/blob/main/LICENSE
[CC-BY-SA 4.0]: https://creativecommons.org/licenses/by-sa/4.0/
[`LICENSE.docs`]: https://github.com/shrine-maiden-heavy-industries/sol/blob/main/LICENSE.docs
[CERN-OHL-P v2]: https://gitlab.com/ohwr/project/cernohl/-/wikis/uploads/3eff4154d05e7a0459f3ddbf0674cae4/cern_ohl_p_v2.txt
...
[issue tracker]: https://github.com/shrine-maiden-heavy-industries/sol/issues
[report an issue]: https://github.com/shrine-maiden-heavy-industries/sol/issues
[discussion forum]: https://github.com/shrine-maiden-heavy-industries/sol/discussions
...
[fork]: https://github.com/shrine-maiden-heavy-industries/sol/fork
...
[`docs`]: https://github.com/shrine-maiden-heavy-industries/sol/blob/main/docs
[testing]: #testing
[new pull request]: https://github.com/shrine-maiden-heavy-industries/sol/compare
